...
bool callAndPopSequence(llvm::Instruction* inst)
{
	...
	Value *rip1, *rip2, *stack1, *stack2;
	if (!(match(inst, m_Store(m_ConstantInt(ci), m_Value(stack1)))
	...
	if (rip1 != rip2 || rip1 != stack1 || !isa<GlobalValue>(rip1))
		return false;

	if (!isa<AllocaInst>(stack2))
		return false;
